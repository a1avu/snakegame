...
/* <해야하는 코드 수정>
  ...
  /* 종윤씨의 메모장 (해결 하면 하나씩 지웁시다)
        
        음식 랜덤 생성 -->>
          - width만큼의 x  height 만큼의 y좌표 값 랜덤으로 받은 후 하나의 변수에 저장하고 출력하게   (성공)
          - 하나의 값에 넣고 해당 좌표에 먹이 생성 1~2개 씩 생성                                    (성공)
          - 머리가 음식에 닿으면 score 올라가고  (snakeMove에 넣어야 함)                            (성공)
          - 꼬리 하나 증가                                                                         (실패)
          - 음식 지워짐(이건 그냥 지나가면 자동으로 초기화 되는것 처럼 보임)                         (성공)
  
        snakeMove     -->>
          - 처음에 이상하게 출력되는거 수정
          - 직진 중일때 반대 방향으로 못 움직이게 수정해야함 ()
          - 뭔가 키보드 입력받고 움직이고 할 때 툭툭 끊김 → clock 함수라는게 있다는데  (나중에 시간 날때 수정하는게 좋을 듯 함)
  */