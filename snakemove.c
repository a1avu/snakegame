...
void inter_face(char nickname[20]);
void map(char nickname[20]);
void moveSnake(Snake *snake, int *snakeLength, Direction direction);
...
void map(char nickname[20]){  //interface에서 nickname 받아와서 사용
    system("cls");
    int i, j;
...
void moveSnake(Snake *snake, int *snakeLength, Direction direction){
    ...
    if(snake[0].x == food_x && snake[0].y == food_y){  
        ...
        // 먹이를 먹지 않았을 때
        // 이전 꼬리의 위치
        preTailX = snake[*snakeLength - 1].x;
        preTailY = snake[*snakeLength - 1].y;
